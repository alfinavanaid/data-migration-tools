...
class SequenceLayer
{
    ...
    function findTheParent($table_name)
    {
        ...
        $table = $this->data_source_object->getValueByTableName($table_name);

        // this table has parent relations ?

        if (!empty($table->getRelationsParentString())) {

            // yes
            $this->log($table_name, 'has parent <br/>');

            foreach ($table->getRelationsParentString() as $relations_parent) {
                ...
                if ($this->checkParentTableWithoutSequence($parent_table)) {
                    ...
                    if ($this->data_source_object[$table_name]->extract_layer == null) {
                        $highest_extract_layer = $this->getHighestParentSequence($table_name);
                        $this->data_source_object[$table_name]->extract_layer = $highest_extract_layer + 1;
            ...
            if ($this->data_source_object[$table_name]->extract_layer == null) {
                $this->data_source_object[$table_name]->extract_layer = 1;
    ...
    function findTheChild($table_name)
    {
        ...
        $table = $this->data_source_object[$table_name];
        ...
        if (count($table->relations_child) > 0) {
            ...
            foreach ($table->relations_child as $relations_child) {
                ...
                if ($this->checkParentTableWithoutSequence($child_table_name)) {
                    ...
                    // no 
                    if ($this->data_source_object[$child_table_name]->extract_layer == null) {
                        $highest_extract_layer = $this->getHighestParentSequence($child_table_name);
                        $this->data_source_object[$child_table_name]->extract_layer = $highest_extract_layer + 1;
    ...
    function checkParentTableWithoutSequence($table_name)
    {
        ...
        foreach ($this->data_source_object[$table_name]->relations_parent as $relations_parent) {
            $extract_layer = $this->data_source_object[Relations::getReferenceTablenameOfString($relations_parent)]->extract_layer;
        ...
        if (count($this->data_source_object[$table_name]->relations_parent) == 0) {
    ...
    function findAnyTableWithoutSequence()
    {
        ...
        foreach ($this->data_source_object as $data_source_object) {
            if ($data_source_object->extract_layer != null && count($data_source_object->relations_child) > 0) {
                foreach ($data_source_object->relations_child as $relations_child) {
                    if (
                        $this->data_source_object[Relations::getReferenceTablenameOfString($relations_child)]->extract_layer == null
                        && count($this->data_source_object[Relations::getReferenceTablenameOfString($relations_child)]->relations_child) > 0
                    ) {
